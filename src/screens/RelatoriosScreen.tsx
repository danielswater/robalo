...
import { useComandas, PaymentMethod } from '../context/ComandaContext';
...
// ✅ Máscara simples: só números -> DD/MM/AAAA (sem data fixa)
...
// Aceita "DD/MM/AAAA" (ou com -) e valida de verdade.
...
export default function RelatoriosScreen() {
  ...
  // inputs do período personalizado (começam VAZIOS)
  ...
  const effectiveRange = useMemo(() => {
    ...
    if (!appliedStart || !appliedEnd) {
      return { start: null as Date | null, endExclusive: null as Date | null, label: 'Selecione um período' };
    }
    ...
    return {
      start,
      endExclusive,
      label: `${formatDate(start)} até ${formatDate(appliedEnd)}`,
    };
  ...
  const subtitle =
    mode === 'CUSTOM' && (!appliedStart || !appliedEnd)
      ...
      : totals.count === 0
        ...
        : `${totals.count} comanda(s) fechada(s) nesse período.`;
  ...
  const applyCustom = () => {
    ...
    if (s.getTime() > e.getTime()) {
      setError('A data início não pode ser depois da data fim.');
      return;
    }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.modeBox}>
        ...
        >
          <Text style={[styles.modeText, mode === 'CUSTOM' && styles.modeTextSelected]}>Período</Text>
        </TouchableOpacity>
      </View>
      ...
      <Text style={styles.periodLine}>Período: {effectiveRange.label}</Text>
      ...
      {mode === 'CUSTOM' && (!appliedStart || !appliedEnd) ? (
        <View style={styles.empty}>
          <Text style={styles.emptyText}>Escolha um período e toque em “Aplicar”.</Text>
        </View>
      ) : closedInRange.length === 0 ? (
        <View style={styles.empty}>
          <Text style={styles.emptyText}>Nenhuma comanda fechada nesse período.</Text>
        </View>
      ) : (
        <FlatList
          data={closedInRange}
